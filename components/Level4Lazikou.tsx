...
const Level4Lazikou: React.FC<LevelProps> = ({ resources, onUpdateResources, onComplete, onFail, onAction }) => {
    ...
    const render = (timestamp: number) => {
        ...
        gameState.current.bullets.forEach((b, index) => {
            ...
            if (dist < hitboxRadius && gameState.current.hitCooldown <= 0) {
                ...
                onAction?.({
                    levelId: 3,
                    actionType: b.type === 'grenade' ? 'hit-grenade' : 'hit-bullet',
                    actionLabel: b.type === 'grenade' ? '被手榴弹命中' : '被子弹命中',
                    success: false,
                    progressGain: 0,
                    resourceDelta: {
                        soldiers: -damage,
                ...
                onUpdateResources((prev: GameResources) => {
                    ...
                    if (next.soldiers <= 0) {
                        ...
                        onAction?.({
                            levelId: 3,
                            actionType: 'breakthrough-failed',
                            actionLabel: '突围失败',
                            success: false,
                            progressGain: 0,
                            resourceDelta: {
