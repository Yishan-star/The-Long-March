...
const Level2JinshaRiver: React.FC<LevelProps> = ({ resources, onUpdateResources, onComplete, onFail, onAction }) => {
  ...
  const handleAction = (action: 'ferry' | 'distract' | 'scout') => {
    ...
    switch (action) {
      case 'ferry':
        ...
        progressGain = crossedAmount;
        note = `船队全力运送，${crossedAmount}% 部队成功抵达对岸。`;
        addLog(note);
        break;
      ...
      case 'distract':
        ...
        progressGain = 8;
        note = "小分队制造佯攻，成功吸引敌军注意！争取到了宝贵时间。";
        addLog(note);
        break;
      ...
      case 'scout':
        ...
        if (Math.random() > 0.4) {
            ...
            progressGain = 6;
            note = "侦察兵在下游村庄找到了一条隐藏的船只！渡河效率提升。";
            addLog(note);
        } else {
            success = false;
            progressGain = 2;
            note = "侦察兵无功而返，但确认了附近没有伏兵。";
            addLog(note);
        }
        break;
    }
    ...
    onAction?.({
      levelId: 2,
      actionType: action,
      actionLabel: action === 'ferry' ? '组织渡江' : action === 'distract' ? '声东击西' : '化装侦察',
      success,
      progressGain,
      resourceDelta: {
    ...
    setEnemyDistance(Math.max(0, Math.min(120, newEnemyDist)));
  };
