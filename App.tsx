...
import {
  ...
  GameResources,
  GameStatus,
  LevelActionEventInput,
  QualityReport,
  SprintTask
} from './types';
import { HISTORICAL_IMAGE_LIBRARY, LEVELS, INITIAL_RESOURCES } from './constants';
...
import AIProjectPanel from './components/AIProjectPanel';
import { Play, RotateCcw, Award, BookOpen } from 'lucide-react';
import { normalizeResources } from './utils/resource';
import { gameEventBus } from './ai/eventBus';
import { runQualityPipeline } from './ai/qualityPipeline';
import { buildSprintTasks, getAiSuggestion } from './ai/projectCopilot';

const initialQualityReport: QualityReport = {
  score: 100,
  issues: [],
  strengths: ['等待行为数据后开始评估。']
};

const initialSuggestion: AiSuggestion = {
  title: 'AI 建议',
  recommendedAction: '开始行动',
  reason: '采集首批行为数据后，建议会自动更新。',
  expectedImpact: '形成可追踪的关卡质量样本。'
};

const initialSprintTasks: SprintTask[] = [
  {
...
const App: React.FC = () => {
  ...
  const qualityReport = useMemo(() => {
    if (actionHistory.length === 0) {
      return initialQualityReport;
    }
    return runQualityPipeline(resources, actionHistory);
  }, [resources, actionHistory]);

  const aiSuggestion = useMemo(() => {
    if (actionHistory.length === 0) {
      return initialSuggestion;
    }
    return getAiSuggestion(currentLevel.id, resources, actionHistory);
  }, [currentLevel.id, resources, actionHistory]);

  const sprintTasks = useMemo(() => {
    if (actionHistory.length === 0) {
      return initialSprintTasks;
    }
    return buildSprintTasks(qualityReport, actionHistory);
  }, [qualityReport, actionHistory]);

  useEffect(() => {
    // [AI-新增] 观察者模式：同一动作事件驱动多个订阅方（历史、实时日志）
    const unsubscribeHistory = gameEventBus.subscribe((event) => {
      setActionHistory((prev) => [...prev, event].slice(-40));
    });

    const unsubscribeFeed = gameEventBus.subscribe((event) => {
      const timeLabel = new Date(event.timestamp).toLocaleTimeString('zh-CN', {
        hour12: false,
        hour: '2-digit',
        minute: '2-digit',
        second: '2-digit'
      });
      const line = `[${timeLabel}] ${event.actionLabel}(${event.success ? '成功' : '受阻'}) +${event.progressGain}%`;
      setEventFeed((prev) => [line, ...prev].slice(0, 6));
    });
  ...
  const handleLevelAction = (input: LevelActionEventInput) => {
  ...
  const containerClass =
    'historical-filter relative w-full h-full md:aspect-[9/16] md:h-[90vh] md:w-auto bg-[#1c1917] overflow-hidden md:rounded-[2rem] shadow-2xl md:border-8 md:border-[#292524] flex flex-col';
  ...
  const renderStartScreen = () => (
    <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center">
      ...
      <div
        ...
        style={{
          backgroundImage: `url(${HISTORICAL_IMAGE_LIBRARY.start})`,
      ...
      <div className="relative z-10 animate-fade-in flex flex-col items-center h-full justify-between py-12 w-full">
        <div className="mt-16 flex flex-col items-center">
          <div className="bg-red-900/80 text-white px-3 py-1 text-xs tracking-[0.3em] uppercase mb-4 border border-red-500/50">
            Historical Game
          </div>
    ...
    return (
      <div className="absolute inset-0 flex flex-col bg-stone-900">
        ...
        <div className="relative z-10 flex-1 overflow-y-auto pt-24 pb-6 px-4">
          ...
          {levelIndex === 2 && <Level4Lazikou {...commonProps} />}
        </div>
      </div>
    );
  };
    ...
    <div className="absolute inset-0 flex flex-col items-center justify-center bg-stone-900/95 text-center p-6">
      <div className="w-full max-w-sm animate-slide-up">
        ...
        <div className="bg-stone-800/50 p-6 border-l-2 border-green-600 mb-8 text-left">
          <p className="text-stone-300 text-sm leading-relaxed">部队成功完成了本阶段的战略转移。休整片刻，准备迎接下一阶段的挑战。</p>
        </div>
  ...
  const renderVictory = () => (
    <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6">
      <div
        className="absolute inset-0 bg-cover bg-center"
        style={{
          backgroundImage: `url(${HISTORICAL_IMAGE_LIBRARY.victory})`,
  ...
  const renderGameOver = () => (
    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black p-6">
      <div
        ...
        style={{ backgroundImage: `url(${HISTORICAL_IMAGE_LIBRARY.gameOver})` }}
      ></div>
